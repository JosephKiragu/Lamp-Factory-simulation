...
function RandomNGs( P::Parameters )
	...
	machine_times[1] = () -> rand(rng, Normal(P.mean_machine_times[1], 7))
...
function stall_event!( system::SystemState, event::Event )
	...
	# println("Stalling happening at time = $(system.time) for event_id = $(event.id) server 1 queue_length = $(length(system.order_queues[1]))")
...
function update!( system::SystemState, P::Parameters, R::RandomNGs, event::Finish )
    server = event.server
    if server < n_servers && length(system.order_queues[server+1]) >= P.max_queue[server+1] # serve
...
function output_files(P::Parameters)
    dir = pwd()*"/lamp_factory_data"*
	"/mean_interarrival" * string(P.mean_interarrival) * # mean_interarrival
	"/seed"*string(P.seed) # directory name
...
function run_lamp_sim(P::Parameters)
	...
	#Print metadata for both outputs
